// ClientApp/src/pages/User/UserAccountPage.tsx
import {type FC, useState, useEffect } from 'react';
import { Card, Typography, Form, Input, Button, Space, notification, Avatar, Modal } from 'antd';
import { UserOutlined, EditOutlined } from '@ant-design/icons';
import PrivateLayout from '../../components/PrivateLayout/PrivateLayout';
import { useAuth } from '../../hooks/useAuth';
import { getUserProfile, updateUserProfile, updateUserEmail, updateUserPassword } from '../../api/users';
...
const UserAccountPage: FC = () => {
    ...
    const handleProfileUpdate = async (values: any) => {
        ...
        try {
            ...
            if (profile) {
                ...
                if (user && user.name !== values.nama_Peminjam) {
                    authLogin(user.token, user.userId, values.nama_Peminjam, user.email, user.role);
                }
            }
        } catch (error: any) {
            notification.error({
    ...
    const handleEmailUpdate = async (values: any) => {
        ...
        try {
            ...
            // Optionally, force re-login or update token if backend re-issues it
                ...
                <Space direction="vertical" size="middle" style={{ width: '100%' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <Space>
                            ...
                            <Avatar size={64} icon={<UserOutlined />} src="https://i.pravatar.cc/150?img=68" /> {/* Placeholder image */}
                    ...
                    <Form
                        ...
                        wrapperCol={{ span: 18 }}
                    >
                        <Form.Item label="Name" name="nama_Peminjam" rules={[{ required: true, message: 'Name is required!' }]}>
                            {editMode ? <Input /> : <Text>{profile.nama_Peminjam}</Text>}
                        </Form.Item>

                        {/* Status, Institute, Studies are not in backend DTO based on schema, so they are not editable/displayable here */}
                        {/* If you want to display these, they need to be added to the backend UserProfileDto and DB schema. */}
                        {/* For now, commenting out as per strict schema adherence. */}
                        {/*
            <Form.Item label="Status">
              <Text>Student</Text> <Button type="text" icon={<EditOutlined />} />
            </Form.Item>
            <Form.Item label="Institute">
              <Text>Pradita University</Text> <Button type="text" icon={<EditOutlined />} />
            </Form.Item>
            <Form.Item label="Studies">
              <Text>Informatics / 2024</Text> <Button type="text" icon={<EditOutlined />} />
            </Form.Item>
            */}
                ...
                <Space direction="vertical" size="middle" style={{ width: '100%' }}>
                    ...
                    >
                        ...
                        <Form.Item label="Phone" name="no_Telp">
                            ...
                            <Button type="text" icon={<EditOutlined />} onClick={() => setEditMode(true)} /> {/* Re-use edit mode for phone/address */}
                        </Form.Item>

                        <Form.Item label="Address" name="alamat">
                            {editMode ? <Input.TextArea autoSize={{ minRows: 2, maxRows: 4 }} /> : <Text>{profile.alamat || 'N/A'}</Text>}
                            <Button type="text" icon={<EditOutlined />} onClick={() => setEditMode(true)} /> {/* Re-use edit mode for phone/address */}
                        </Form.Item>
